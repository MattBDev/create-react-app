...
import { bold, cyan, red, yellow, green, underline } from 'picocolors';
...
import { getCompilerHooks } from './ForkTsCheckerWebpackPlugin';
...
function printInstructions(appName, urls, useYarn) {
  ...
  if (urls.lanUrlForTerminal) {
    console.log(
      `  ${bold('Local:')}            ${urls.localUrlForTerminal}`,
    );
    console.log(
      `  ${bold('On Your Network:')}  ${urls.lanUrlForTerminal}`,
    );
  } else {
...
function prepareProxy(proxy, appPublicFolder, servedPathname) {
  ...
  if (typeof proxy !== 'string') {
    ...
    console.log(
      red('Instead, the type of "proxy" was "' + typeof proxy + '".'),
    );
    console.log(
  ...
  function mayProxy(pathname) {
    const maybePublicPath = _resolve(
      appPublicFolder,
...
function choosePort(host, defaultPort) {
  return detect(defaultPort, host).then(
    ...
    err => {
      throw new Error(
        red(`Could not find an open port at ${bold(host)}.`) +
          '\n' +
