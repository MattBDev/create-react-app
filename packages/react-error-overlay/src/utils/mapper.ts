 ...
 * @returns {Promise<StackFrame[]>} A promise which resolves to a set of <code>StackFrame</code>s enhanced with their original positions and code (when available).
 */
export async function map(frames: StackFrame[], contextLines: number = 3): Promise<StackFrame[]> {
  const cache = {};
  const files: string[] = [];
